...
async function getEncodedAssetData(asset: AssetTypeForm) {
  const res = await client.post<AssetEncodedTypeForm>(
    "protocol/ethereum/order/indexer/v0.1/encoder/assetType",
    asset
  );
  return res.data;
}
...
async function getEncoderData(data: OrderDataForm) {
  const res = await client.post<AssetEncodedTypeForm>(
    "protocol/ethereum/order/indexer/v0.1/encoder/data",
    data
  );
  return res.data;
}

async function prepareOrderMessage(
  form: Omit<OrderForm, "signature">
): Promise<Order> {
...
function createTestOrder(maker: string): Omit<OrderForm, "signature"> {
  return {
    maker: maker,
    make: {
      type: {
...
async function putOrder(order: OrderForm) {
  const res = await client.put(
    "/protocol/ethereum/order/indexer/v0.1/orders",
    order
  );
  return res.data;
}

async function signOrderForm(
  form: Omit<OrderForm, "signature">
): Promise<OrderForm> {
